...
def calculateMCStanimoto(ref_mol, target_mol):
    ...
    numCommonAtoms = rdFMCS.FindMCS(
        pair_of_molecules,
        atomCompare=rdFMCS.AtomCompare.CompareElements,
        bondCompare=rdFMCS.BondCompare.CompareOrderExact,
        matchValences=True,
    ).numAtoms
    mcsTanimoto = numCommonAtoms / (
        (numAtomsTargetCpd + numAtomsRefCpd) - numCommonAtoms
    )
...
def replace_dummies(smi1):
    ...
    frag1_A = Chem.rdmolops.ReplaceSubstructs(
        frag1_not_cleaned, Chem.MolFromSmarts("[Yb]"), chain
    )
    frag1_B = Chem.rdmolops.ReplaceSubstructs(
        frag1_A[0], Chem.MolFromSmarts("[Lu]"), methyl
    )
    frag1 = Chem.rdmolops.ReplaceSubstructs(
        frag1_B[0], Chem.MolFromSmarts("[Ta]"), methyl
    )
...
def delete_dummies(smi):
    mol_a = AllChem.DeleteSubstructs(
        Chem.MolFromSmiles(smi), Chem.MolFromSmarts("[Yb]")
    )
...
def similarity(smi1, smi2, partialCharges, methodPsi4, basisPsi4):
    ...
    if number_dummies_1 != number_dummies_2:
        ...
        (
            simShape,
            frag1_with_anchor_aligned,
            frag2_with_anchor_aligned,
        ) = EmbedAlignConstrainedScore(frag1_with_anchor_hs, frag2_with_anchor, core())
